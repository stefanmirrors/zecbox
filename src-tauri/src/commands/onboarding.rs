...
pub async fn complete_onboarding(
    ...
    let stealth_active = matches!(privacy_mode, PrivacyMode::Stealth | PrivacyMode::Shield);

    // Mark onboarding complete and persist privacy choice
    ...
    // If stealth/shield mode chosen, start Arti + enable firewall before starting node
    if stealth_active {
        ...
        if let Err(e) = tor::verify_tor_path().await {
            ...
            return Err(format!("Stealth Mode failed traffic verification: {}. Try again or select Standard.", e));
        }

        log::info!("Stealth Mode enabled during onboarding");
    }

    // Start zebrad (reads stealth/proxy status to generate correct config)
